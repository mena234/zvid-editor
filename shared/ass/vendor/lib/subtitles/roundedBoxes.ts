...
import { formatTime, captionSeparators } from '../../utils/subtitles'
import { segmentWords } from '../../utils/subtitleText'
import { visualWordSpans, type WordSpan } from '../../utils/subtitleBidi'
...
import { loadRenderFont, type LoadedFont } from '../../../fontMetrics'
import { fontCoversText } from '../../../../fontResolution'
import { createSubtitleMeasurer, type SubtitleMeasure } from '../../utils/subtitleFontMetrics'
...
export async function loadSubtitleFontMetrics(styles: SubtitleStyles, fonts: LoadedFont[] = []): Promise<FontMetrics> {
  if (fonts.length && typeof document !== 'undefined') {
    const measured = fonts.map((font) => {
      const ctx = document.createElement('canvas').getContext('2d')!
      ctx.font = `${(styles.fontSize ?? 50) * font.scale}px "${font.alias}"`
      return { ...font, measure: (text: string) => ctx.measureText(text).width }
    })
    return { measure: createSubtitleMeasurer(measured, styles.fontFamily ?? '', fontCoversText) }
  }
...
export function layoutGroup(
  ...
  playResX: number,
  playResY: number,
  separators: string[] = texts.map((_, i) => (i ? ' ' : ''))
): LineLayout[] {
  const fontSize = styles.fontSize ?? 50;
  ...
  const starts: number[] = [];
  let fullText = separators[0] ?? '';
  texts.forEach((text, i) => {
    if (i) fullText += separators[i];
  ...
  // the calibrated measurement + scale headroom) — replicate exactly so the
  ...
  if (SCALE_HEADROOM[mode]) {
    lineIdx = wrapWordIndicesByWidth(
      texts,
      wrapWidth,
      fontSize,
    ...
    let current: number[] = [];
    texts.forEach((_, i) => {
      if (/\r?\n/.test(separators[i]) && current.length) {
        lineIdx.push(current);
        current = [];
      }
      const candidate = [...current, i];
      const w = measureIndices(candidate);
...
export default async function prepareRoundedBoxes(
  ...
  const wantBgBox = !!styles.background && (styles.backgroundRadius ?? 0) > 0;
  const wantActiveBox =
  ...
  for (let ci = 0; ci < subtitle.captions.length; ci++) {
    ...
    const texts = isWordMode
      ? words.map((w) => w.text)
      : segmentWords(String(caption.text ?? ''));
    ...
    if (wantActiveBox && isWordMode) {
      ...
      for (const line of lines) {
        for (let k = 0; k < line.words.length; k++, flatIndex++) {
          ...
          const nextStart =
            flatIndex < words.length - 1
              ? words[flatIndex + 1].start
              : groupEnd;
          for (const span of line.wordSpans[k]) {
            events += boxDialogue(
              'ZvidBoxActive',
              w.start,
              nextStart,
              roundedRectPath(
                line.left + span.left - pad,
                line.top - pad,
                span.width + 2 * pad,
                fontSize + 2 * pad,
                activeRadius
  ...
  const boxFont = String(styles.fontFamily ?? 'Poppins')
    .split(',')[0]
    .trim();
  const styleLines: string[] = [];
  if (wantBgBox)
    styleLines.push(
      boxStyleLine('ZvidBoxBg', styles.background as string, boxFont)
    );
  if (wantActiveBox)
    styleLines.push(
      boxStyleLine(
        'ZvidBoxActive',
        styles.activeWord!.background as string,
        boxFont
      )
    );
