...
function generateDirectionalFill(
  ...
  const lineStarts = new Set(
    lines.slice(1).map((line) => words.indexOf(line.words[0]))
  );
  const textFor = (active = -1) =>
    words
      .map((word, i) => {
        ...
        return (
          gap + (i === active ? `{\\alpha&H00&}${text}{\\alpha&HFF&}` : text)
        );
      })
  ...
  for (const line of lines)
    for (let i = 0; i < line.words.length; i++) {
      ...
      const nextStart =
        index + 1 < words.length ? words[index + 1].start : word.end;
      const duration = Math.max(1, Math.round((nextStart - word.start) * 1000));
      const spans = line.wordSpans[i]
        .slice()
...
export function generateASSContent(
  ...
  groups.forEach((caption, groupIndex) => {
    ...
    if (hasRtlText(caption.text ?? group.map((word) => word.text).join(' '))) {
      if (!measure)
        throw new Error('RTL subtitle fill requires calibrated font metrics');
